    ...
    <div>
      ...
      <div className='mx-auto w-full max-w-[400px] bg-background py-3'>
        ...
        <div className="mt-4 mb-3">
          <TextInput type="text" placeholder="アカウント名/事業者名/地域 でフィルタリング" onChange={(e) => setFilter(e.target.value)}/>
        </div>
        <div className="border-border rounded-lg border-solid border-2 box-border p-2">
          <div className="overflow-y-scroll max-h-[550px]">
            <table className="w-full">
              ...
              {users.filter(user=>user.area.toLocaleLowerCase().includes(filter.toLocaleLowerCase()) || user.name.toLocaleLowerCase().includes(filter.toLocaleLowerCase()) || user.user.toLocaleLowerCase().includes(filter.toLocaleLowerCase())).map((user, i)=>(
                <tr className={"border-t-border border-t-2 h-12" + (i % 2 == 0 ? "" : " bg-[#d4d4d4]")} key={user.user}>
                  <td>{user.user}</td>
                  <td>{user.name}</td>
                  <td>{user.area}</td>
                  <td>
                    <button type="button" className="bg-danger px-2 py-1 rounded-lg text-background font-bold" onClick={() => onClickDelete(user.user, user.area, user.name)}>
                        削除
                    </button>
                  </td>
                </tr>

    ...
    <div>
      ...
      <div className='mx-auto w-full max-w-[400px] bg-background py-3'>
        ...
        <div className="mt-4 mb-3">
          <TextInput type="text" placeholder="事業者名/地域名/固有番号 でフィルタリング" onChange={(e) => setFilter(e.target.value)}/>
        </div>
        <div className="border-border rounded-lg border-solid border-2 box-border p-2">
          <div className="overflow-y-scroll max-h-[550px]">
            <table className="w-full">
              ...
              {traders.filter(trader=>trader.area?.toLocaleLowerCase().includes(filter.toLocaleLowerCase()) || trader.name.toLocaleLowerCase().includes(filter.toLocaleLowerCase()) || trader.code.toLocaleLowerCase().includes(filter.toLocaleLowerCase())).map((trader, i)=>(
                <tr className={"border-t-border border-t-2 h-12" + (i % 2 == 0 ? "" : " bg-[#d4d4d4]")} key={trader.code}>
                  <td>{trader.name}</td>
                  <td>{trader.area}</td>
                  <td>{trader.code}</td>
                  <td>
                    <button type="button" className="bg-danger px-2 py-1 rounded-lg text-background font-bold" onClick={() => onClickDelete(trader.name, trader.area, trader.code)}>
                        削除
                    </button>
                  </td>
                </tr>

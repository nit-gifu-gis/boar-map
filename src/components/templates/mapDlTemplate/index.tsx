...
const MapDLTemplate: React.FunctionComponent = () => {
  ...
  const indexNo = useMemo(() => {
    const d = fileInfo
      .map((info, i) => (info.mesh.indexOf('インデックス') !== -1 ? i : -1))
      .find((no) => no !== -1);
  ...
  const meshGroups = useMemo(() => {
    ...
    const prefixInfo: {
      [key: string]: {
        min: number;
        max: number;
        values: { key: number | string; label: string; file_no: number }[];
      };
    } = {};
    fileInfo.forEach((info) => {
      ...
      if (!prefixList.includes(prefix)) {
        ...
        prefixInfo[prefix] = {
          min: prefix === 'その他' ? 0 : (no as number),
          max: prefix === 'その他' ? 0 : (no as number),
          values: [],
        };
      }
        ...
        <div className='my-3 mx-3 max-w-[500px]'>
          {!fileInfo.length ? (
            ...
            <div className='box-border w-full rounded-[10px] border-2 border-solid border-border py-[10px] px-3'>
              <div className='text-2xl font-bold'>ダウンロード中...</div>
              <div>ダウンロード完了までしばらくお待ちください...</div>
            </div>
          ) : (
            <div className='box-border w-full rounded-[10px] border-2 border-solid border-border py-[10px] px-3'>
              <span className='font-bold'>
                ダウンロードしたい地図のメッシュ番号を選択してください。
              </span>
              <ul className='list-inside list-disc'>
                {currentPrefix === null ? (
                  <>
                    {indexNo !== -1 ? (
                      <li
                        className='my-1 cursor-pointer text-xl text-accent underline'
                        onClick={() => onClickDownload(fileInfo[indexNo].file_no)}
                      >
                        インデックス
                      </li>
                    ) : undefined}
                    {meshGroups.prefixList.map((prefix) => (
                      ...
                      >
                        {prefix === 'その他'
                          ? 'その他'
                          : `${prefix}-${meshGroups.prefixInfo[prefix].min} ～ ${prefix}-${meshGroups.prefixInfo[prefix].max}`}
                      </li>
                    ))}
                  </>
                ) : (
                  <>
                    {meshGroups.prefixInfo[currentPrefix].values.map((value) => (
                      <li
                        key={value.label}
                        className='my-1 cursor-pointer text-xl text-accent underline'
                        onClick={() => onClickDownload(value.file_no)}
                      >
                        {value.label}
                      </li>
                    ))}

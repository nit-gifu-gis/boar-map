...
import { MeshNoInputProps, MeshNoInputReqHeader } from "./interface";
...
const MeshNoInput: React.FunctionComponent<MeshNoInputProps> = (props) => {
  ...
  const initForm = async () => {
    try {
      ...
      // 表示をセット
      (document.getElementById(props.id + "City") as HTMLInputElement).value =
        city;
    } catch (error) {
      console.error(error);
    }
  ...
  useEffect(() => {
    if (props.defaultValue != null) {
      ...
      (document.getElementById(props.id + "City") as HTMLInputElement).value =
        cityValue;
      (document.getElementById(props.id + "Num") as HTMLInputElement).value =
        numValue;
    } else {
  ...
  const onChangeValue = () => {
    // 入力情報からメッシュIDを作る
    const cityValue = (
      document.getElementById(props.id + "City") as HTMLInputElement
    ).value;
    const numValueRow = (
      document.getElementById(props.id + "Num") as HTMLInputElement
    ).value;
    // 有効な桁数の時のみセット

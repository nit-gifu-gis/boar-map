...
const ImageInput: FC<ImageInputProps> = (props) => {
  ...
  const [objURLs, setObjURLs] = useState(
    props.objectURLs == null ? [] : props.objectURLs,
  );
  const [imgIDs, setImgIDs] = useState(
    props.imageIDs == null ? [] : props.imageIDs,
  );

  if (form_id == "") {
    setFormId(Math.random().toString(36).slice(2, 10));
  }

  const makeCompressedImageURL = async (
    file: File,
  ): Promise<ImagewithLocation> => {
  ...
  const formChanged = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setError("");
    if (
      objURLs.length + imgIDs.length + e.target.files.length >
      props.max_count
    ) {
  ...
  const onClickSelect = () => {
    const fileForm = document.getElementById(
      "file_" + form_id,
    ) as HTMLInputElement;
  ...
  const onClickDeleteRemoteImage = async (index: number) => {
    if (await confirm("この画像を削除しますか？")) {
      ...
      if (props.onServerImageDeleted != null)
        props.onServerImageDeleted(newList);
      setImgIDs(newList);
    }
  };
    ...
    const previewChild = (
      <div className="img-preview relative w-full" key={"Uploaded-" + (i + 1)}>
        <img
          ...
          src={
            SERVER_URI + "/Image/GetImage?id=" + imgIDs[i] + "&token=" + token
          }
        ...
        <button
          type="button"
          className="shadow-2 absolute -right-[5px] -top-[5px] box-border h-[30px] w-[30px] rounded-[30px] bg-background text-center text-xl font-bold text-text"
  ...
  return (
    ...
    >
      ...
      <div className="w-full text-center">
        <Button color="primary" onClick={onClickSelect.bind(this)}>
          画像を選択
        </Button>
      </div>
    </div>

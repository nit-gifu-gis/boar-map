...
import { CityInputProps, CityInputReqHeader } from "./interface";

const CityInput: React.FunctionComponent<CityInputProps> = ({
  defaultValue,
  id,
  lat,
  lng,
  ...
  const initForm = () => {
    const frm = document.getElementById(id) as HTMLSelectElement;
    if (frm == null) return;
  ...
  const getCityName = (): Promise<string> => {
    return new Promise<string>((resolve, reject) => {
      if (!lat || !lng) reject("Location not found.");

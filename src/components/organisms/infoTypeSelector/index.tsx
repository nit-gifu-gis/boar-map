...
const InfoTypeSelector: React.FunctionComponent<InfoTypeSelectorProps> = ({ onChanged }) => {
  ...
  return (
    <form>
      {options == null ? <></> : options.map((o,i) => (
        <div key={o.alt} className={"w-full h-9 mb-10 " + (i != 0 ? "mt-5" : "mt-10")}>
          <input id={`radio-${i + 1}`} type="radio" name="radio" onChange={() => {setSelected(o.value); onChanged(o.value);}} className="hidden"/>
          <label htmlFor={`radio-${i + 1}`} className="flex items-center mx-5">
            <div className="mr-3">
              <RoundedCheck checked={selected == o.value} />
            </div>
            <InfoTypeItem src={o.icon} alt={o.alt} text={o.label} width={o.resolution[0]} height={o.resolution[1]} selected={selected == o.value} />
          </label>
        </div>
      ))}
    </form>

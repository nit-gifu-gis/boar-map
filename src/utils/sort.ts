import { BoarFeatureV2, FeatureBase, ReportFeature, TrapFeature, VaccineFeature } from "../types/features";

export const sortFeatures = (key: string, features: BoarFeatureV2[] | TrapFeature[] | ReportFeature[] | VaccineFeature[], desc: boolean): BoarFeatureV2[] | TrapFeature[] | ReportFeature[] | VaccineFeature[] => {
  return features.sort((a, b) => {
    const getItem = (feature: FeatureBase, key: string): string | Date | number => {
      ...
      if (key === "ID$"
        ...
        // 空文字の場合は一番下になるように
        if (props[key] === "")
          return desc ? Number.MIN_VALUE : Number.MAX_VALUE;

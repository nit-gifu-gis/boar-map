...
export const filterListByArea = (list?: TraderList, area?: string): TraderInfo[] => {
  if (!list) return [];
...
export const fetchTraderList = async (): Promise<TraderList> => {
  const inf = await fetch(SERVER_URI + '/Jibie/List', {
    headers: {
...
export const getTraderByName = async (name: string, area?: string): Promise<TraderInfo | null> => {
  const list = await fetchTraderList();
  if (area != undefined && area != null && !area && area !== ' ' && !list.area.includes(area))
    return null;
...
export const hasTrader = async (boars: Record<string, unknown>[]): Promise<boolean> => {
  const myInfo = await fetchTraderInfo();
  if (!myInfo) return false;

  for (let i = 0; i < boars.length; i++) {
    if (
      boars[i]['処分方法'] === '利活用（ジビエ利用）' ||
      boars[i]['処分方法'] === 'ジビエ業者渡し'
    ) {
      if (boars[i]['地域'] === myInfo.area && boars[i]['ジビエ業者'] === myInfo.info?.name)
        return true;
    }
  }
  return false;
...
export const fetchTraderInfo = async (): Promise<MyTraderInfo> => {
  const inf = await fetch(SERVER_URI + '/Jibie/Me', {
    headers: {
      'X-Access-Token': getAccessToken(),
    },
  });
...
export const includeTrader = (list: TraderInfo[], value: TraderInfo): boolean => {
